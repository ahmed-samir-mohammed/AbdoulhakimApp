...
export class NavbarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._service.getAllBar(this.filter).subscribe((res) => {
      if (res.isSuccess) {
    ...
    this.date = new Intl.DateTimeFormat('ar-EG-u-ca-islamic', {
      day: 'numeric',
      month: 'long',
      weekday: 'long',
      year: 'numeric',
    ...
    this.date1 = new Intl.DateTimeFormat('ar-EG', {
      day: 'numeric',
      month: 'long',
      year: 'numeric',
    }).format(Date.now());
  ...
  routeBar(category) {
    ...
    this._serviceMedia.addVisitCount(category).subscribe((res) => {
      if (res.isSuccess) {
        // this.data = res.data;
        // this.totalRecordsCount = res.totalRecordsCount;
        // this.pageCount = res.pageCount > 5 ? 5 : res.pageCount;
